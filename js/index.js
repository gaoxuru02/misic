...
//获取歌词
	function getlyric(){
		...
		$.post('http://api.jirengu.com/fm/getLyric.php', {sid: Sid})
				...
				if (lyr.lyric) {
					...
					if(line != ""){
						for(var i in line){//遍历歌词数组
							...
							value =value.replace(/by/g,"");
							value = value.replace(/饥人谷/g,"");
							//console.log(value);
			...
			$('.music-lyric .lyric').html("<li>本歌没有找到歌词</li>");
		})
	}
	//拼接歌词，并挷定数据
	function renderLyric(){
		...
		var num  = parseInt(myAudio.duration);
		var  second =parseInt(num/60) ;
		var  mim =parseInt( num%60);
		var bb = parseInt(mim/10);
		var  aa = mim % 10;
		console.log(parseInt(myAudio.duration));
		console.log(parseInt(second));
		console.log(parseInt(mim));
		$(".num8").text(aa );
		$(".num7").text(bb );
		$(".num6").text(second );
	}
	...
	function showLyric(){
		...
		for(var i=0;i< lyricArr.length;i++){//遍历歌词下所有的li
			var curT = $(".lyric li").eq(i).attr("data-time");//获取当前歌词时间
			var nexT = $(".lyric li").eq(i+1).attr("data-time");//下句歌词时间
			var curTime = myAudio.currentTime;
			if ((curTime > curT) && (curT < nexT)){
				//当下一句时间和歌曲当前时间之间的时候 就渲染 并滚动
